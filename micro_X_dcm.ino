...
void fast_Loop()
{
  ...
  //YAW
 pid_yaw = ToDeg(Omega[2]); 

  
  
  //Throttle
throttle = constrain(rc[0]-1010,0,255);
  if(rc[0]>1500)
  {
    throttle=0;
  }
...
analogWrite(3, constrain(throttle-pid_pitch+pid_roll+pid_yaw,0,255));//arrière droit
analogWrite(9, constrain(throttle+pid_pitch-pid_roll+pid_yaw,0,255));//avant gauche
analogWrite(10, constrain(throttle-pid_pitch-pid_roll-pid_yaw,0,255));//arrière gauche
analogWrite(11, constrain(throttle+pid_pitch+pid_roll-pid_yaw,0,255));//avant droit
